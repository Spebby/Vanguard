...
public class EnemySpawner : MonoBehaviour
{
    ...
    [Range(0f, 1f)] public float zigzagPatternChance = 0.15f;
    [Tooltip("Spawn enemies only on outer lanes (edges pattern)")]
    [Range(0f, 1f)] public float edgesPatternChance = 0.1f;
    ...
    [Tooltip("Random forward/backward offset range for natural traffic spacing")]
    ...
    private List<GameObject>[] laneEnemies;
    private float gameStartTime;
    private bool isMultiSpawn = false;
    
    void Start()
    {
        // Initialize lane tracking
        laneEnemies = new List<GameObject>[5];
        for (int i = 0; i < 5; i++)
        {
            laneEnemies[i] = new List<GameObject>();
        }
    ...
    void CreateLanes()
    {
        for (int i = 0; i < 5; i++)
        {
            ...
            lane.transform.position = new Vector3((i - 2) * laneSpacing, spawnHeight, 0);
    ...
    void SpawnEnemies()
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            if (laneEnemies[i].Count < maxEnemiesPerLane)
            {
        ...
        isMultiSpawn = lanesToSpawn.Count > 1;
    ...
    float GetPositionVariation()
    {
        // Apply variation only if enabled and conditions are met
        if (offsetOnlyMultiSpawns && !isMultiSpawn)
        {
            return 0f;
        }
        
        return Random.Range(minPositionOffset, maxPositionOffset);
    }
    ...
    List<int> GetSpawnPattern(List<int> availableLanes)
    {
        ...
        if (!usePatterns || availableLanes.Count < 2)
        {
            ...
            int spawnCount = Random.value < multiLaneSpawnChance ? 
                Random.Range(minSimultaneousSpawns, maxSimultaneousSpawns + 1) : 1;
    ...
    void SpawnEnemyInLane(int laneIndex, float yOffset = 0f)
    {
        ...
        // Track enemy
        laneEnemies[laneIndex].Add(newEnemy);
    }
    
    void CleanupDestroyedEnemies(int laneIndex)
    {
        for (int i = laneEnemies[laneIndex].Count - 1; i >= 0; i--)
        {
            if (laneEnemies[laneIndex][i] == null)
            {
                laneEnemies[laneIndex].RemoveAt(i);
            }
        }
    }
}